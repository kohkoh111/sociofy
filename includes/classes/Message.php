  ...
  public function getMessages($otherUser){
	...
	public function getConversations(){
		...
		foreach($conversations as $username){
			...
			$return_string .= "<a href='messages.php?u=$username'><div class='user_found_messages'>
												...
												margin:0;'> ". $latest_message_details[0]. $split. " </p>
												</div>
												</a>";
			}
	...
	public function getConversationsDropdown($data, $limit){
		...
		foreach($conversations as $username){
			...
			$is_unread_array = mysqli_query($this->con, "SELECT opened FROM messages WHERE user_to='$userLoggedIn' AND user_from='$username' ORDER BY id DESC");
			$row = mysqli_fetch_array($is_unread_array);
